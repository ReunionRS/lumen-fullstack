...
const toProject = (row) => ({
  ...
  floors: safeNum(row.floors),
  projectType: row.project_type,
  areaSqm: safeNum(row.area_sqm),
  estimatedCost: safeNum(row.estimated_cost),
  catalogHouseId: row.catalog_house_id || '',
  catalogHouseName: row.catalog_house_name || '',
  catalogHouseUrl: row.catalog_house_url || '',
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO projects (
        ...
        thumbnail_url, materials, floors, project_type, area_sqm, estimated_cost, catalog_house_id, catalog_house_name,
        catalog_house_url, contract_amount, paid_amount, next_payment_date,
      ...
      [
        ...
        Math.trunc(safeNum(payload.floors)),
        String(payload.projectType == null ? 'typical' : payload.projectType),
        safeNum(payload.areaSqm),
        safeNum(payload.estimatedCost),
        payload.catalogHouseId || null,
        payload.catalogHouseName || null,
        payload.catalogHouseUrl || null,
...
app.patch('/api/projects/:id', authRequired, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE projects SET
      ...
      estimated_cost=$13,
      catalog_house_id=$14,
      catalog_house_name=$15,
      catalog_house_url=$16,
      contract_amount=$17,
      paid_amount=$18,
      next_payment_date=$19,
      last_payment_date=$20,
      status=$21,
      start_date=$22,
      planned_end_date=$23,
      actual_end_date=$24,
    ...
    [
      ...
      Math.trunc(safeNum(merged.floors)),
      merged.projectType,
      safeNum(merged.areaSqm),
      safeNum(merged.estimatedCost),
      merged.catalogHouseId || null,
      merged.catalogHouseName || null,
      merged.catalogHouseUrl || null,
